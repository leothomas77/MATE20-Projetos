...
#define _APP "Normal Mapping Without Precompuuted Tangents 1.0 - Leonardo Thomas"
//#define PRECOMPUTE_TANGENTS
...
void renderQuad();
void renderQuadWithoutTangents();
...
int main()
{
    ...
    // -------------------------
	#if defined PRECOMPUTE_TANGENTS
		Shader shader("shaders/normal_mapping.vs", "shaders/normal_mapping.fs");
	#else
		Shader shader("shaders/NM_without_tangents.vs", "shaders/NM_without_tangents.fs");
	#endif
    // texturas
    // -------------
    ...
    // -----------
	unsigned int fps = 0;
	float lastFrameFPS = 0.0;
	float deltaTimeFPS = 0.0;
	...
	while (!glfwWindowShouldClose(window))
    {
		...
		showFPS(currentFrame, lastFrameFPS, deltaTimeFPS, fps, window);
        // -----
		...
		shader.setBool("showNormal", showNormal);
